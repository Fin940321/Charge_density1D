...
def calculate_charge_bins(
    positions_z: np.ndarray, charges_array: np.ndarray, z_min: float, z_max: float, bins: int
) -> np.ndarray:
    """Vectorized accumulation of charges into spatial bins using numpy histogram."""
    charge_density, _ = np.histogram(
        positions_z, 
        bins=bins, 
        range=(z_min, z_max), 
        weights=charges_array
    )
...
def main() -> None:
    ...
    # 預先分配記憶體以避免重複分配
    positions_z_buffer = np.empty(electrolyte_group.n_atoms, dtype=np.float64)

    print("\n=== Processing Trajectory Sequentially ===")
    start_time = time.time()
    for frame_index in tqdm(frame_indices, desc="Processing frames", unit="frame"):
        universe.trajectory[frame_index]
        # 直接寫入預分配的緩衝區，避免重複創建數組
        positions_z_buffer[:] = electrolyte_group.positions[:, 2]
        frame_charge = calculate_charge_bins(
            positions_z_buffer, charges_array, z_min, z_max, bins
        )
    ...
    print("\n=== Charge Density Results (first/last 5 bins) ===")
    for idx in range(min(5, len(charge_density))):
        print(f"{idx:03d}  z={z_positions[idx]:8.3f}  rho={charge_density[idx]:12.6e}")
    if len(charge_density) > 10:
        print("...")
        for idx in range(max(5, len(charge_density) - 5), len(charge_density)):
            print(f"{idx:03d}  z={z_positions[idx]:8.3f}  rho={charge_density[idx]:12.6e}")
