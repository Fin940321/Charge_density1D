...
print("=" * 70)
print("    自動離子偵測版電荷密度分析 (V6 - Auto Ion Detection)")
print("=" * 70)
print()
...
framestart = int(total_frames / 2)  # 從總幀數的一半開始
frameCount = total_frames - framestart  # 剩餘的幀數
...
print()
print("=== 自動計算的幀數參數 ===")
print(f"  起始幀 (framestart): {framestart}")
print(f"  分析幀數 (frameCount): {frameCount}")
print(f"  結束幀 (frameend): {frameend}")
print(f"  分析時間範圍: {u.trajectory[framestart].time:.2f} - {u.trajectory[frameend-1].time:.2f} ps")
print()
...
### Extract atom indices for different species with auto-detection
print("=== 自動偵測離子類型並提取原子索引 ===")

# 列出 OpenMM 拓撲中所有獨特的殘基名稱
unique_residues = set()
for res in simmd.topology.residues():
    unique_residues.add(res.name)
print(f"  OpenMM 拓撲中的殘基類型: {sorted(unique_residues)}")
print()
...
for cat_type in cation_types:
    if cat_type in unique_residues:
        namecat = cat_type
        print(f"  ✓ 偵測到陽離子類型: {namecat}")
        break

# 自動偵測陰離子
namean = None
for an_type in anion_types:
    if an_type in unique_residues:
        namean = an_type
        print(f"  ✓ 偵測到陰離子類型: {namean}")
        break

# 檢查是否成功偵測到離子
if namecat is None:
    raise ValueError(f"❌ 無法偵測到陽離子！OpenMM 拓撲中的殘基: {sorted(unique_residues)}")
if namean is None:
    raise ValueError(f"❌ 無法偵測到陰離子！OpenMM 拓撲中的殘基: {sorted(unique_residues)}")

print(f"  → 將使用 {namecat} (陽離子) 和 {namean} (陰離子) 進行分析")
print()
...
data_cat = np.loadtxt("hist_q_cat_V6.dat")
data_an = np.loadtxt("hist_q_an_V6.dat")
data_solv = np.loadtxt("hist_q_solv_V6.dat")
data_solution = np.loadtxt("hist_q_solution_V6.dat")
...
# 動態生成圖例標籤
cation_label = f"Cation ({namecat})"
anion_label = f"Anion ({namean})"

# Create figure with better layout
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), dpi=300)

# Upper panel: Individual species
ax1.plot(z_cat, q_cat, label=cation_label, color='blue', linewidth=1.5, alpha=0.8)
ax1.plot(z_an, q_an, label=anion_label, color='red', linewidth=1.5, alpha=0.8)
...
fig.suptitle(f'Charge Density Analysis (V6 - {namecat}/{namean})', 
             fontsize=14, fontweight='bold', y=0.995)

plt.tight_layout()
plt.savefig("charge_density_distribution_V6_auto.png", dpi=600, bbox_inches='tight')
print("  ✓ 已儲存圖表: charge_density_distribution_V6_auto.png")

print()
print("=" * 70)
print("✓ 所有處理完成！(V6 自動離子偵測版本)")
print(f"⚡ 使用 NumPy 向量化獲得最佳性能")
print(f"📊 自動從總幀數的一半 (幀 {framestart}) 開始分析")
print(f"🔍 自動偵測離子類型: {namecat} (陽離子) / {namean} (陰離子)")
print("=" * 70)
